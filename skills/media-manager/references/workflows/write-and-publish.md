...
- 外源素材格式未验证时，**禁止**直接进入 Step 5；见 [orchestration.md § 外源成稿格式准入](../orchestration.md#外源成稿格式准入)。
...
> ⛔ **硬门禁**：本节第一步不是跑 CLI，而是输出 [orchestration.md § 发布硬门禁](../orchestration.md#发布硬门禁step-5--publish-only-step-3-强制执行) 中的 **「发布平台确认」块** 并**等待用户回复「确认发布」**。未完成则 **STOP**。
