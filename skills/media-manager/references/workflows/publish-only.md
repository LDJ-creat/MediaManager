...
详见 [orchestration.md § 外源成稿格式准入](../orchestration.md#外源成稿格式准入)。
...
用户**未明确**时，按 [orchestration.md § 平台选择协议](../orchestration.md#平台选择协议) 执行：
...
> ⛔ **硬门禁**：运行任何 `media * post*` 前，必须输出 [orchestration.md § 发布硬门禁](../orchestration.md#发布硬门禁step-5--publish-only-step-3-强制执行) 中的 **「发布平台确认」块**，并**等待用户回复「确认发布」**。未完成则 **STOP**。
